...
- [x] Select packs interactively or with repeatable `--pack`, supporting `--yes`,
      `--dry-run`, and JSON output.
- [x] Validate and apply core-owned declarative init recipes at the Git root or
      explicit target, preserving unrelated root `dv.yaml` content.
- [x] Create the empty tracked Work backlog layout with no sample work items.
- [x] Reject unsafe paths, unclaimed config writes, config/output collisions, and
      symlink escapes; roll back a failed pack independently.
- [x] Document the static installed-package descriptor convention and add focused
      tests.

## Acceptance Criteria

- [x] `DocPackRegistry` remains catalog-only and no extension code is executed.
- [x] The Work recipe creates only the tracked `backlog/` layout and targeted
      `dv.yaml` configuration.
- [x] Focused tests, typecheck, documentation lint, and backlog validation pass.
