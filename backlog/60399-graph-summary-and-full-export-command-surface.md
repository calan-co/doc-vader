...
- [x] Add `summary` subcommands for `dv work graph` and `dv wi graph`.
- [x] Add `export` subcommands for `dv work graph` and `dv wi graph`.
- [x] Make summary default to human-readable table output and support `--format json`.
- [x] Make export default to `--format json` and support `--format dot`.
- [x] Define and document a stable full-graph export JSON contract with summary
      metadata, nodes, edges, and diagnostics.
- [x] Keep JSON and DOT rendering behind the graph output extension seam rather
      than command-local serializers.
- [x] Preserve existing `nodes`, `edges`, and `inspect` commands.
- [x] Add focused tests for summary output, full export JSON, full export DOT,
...
- [x] `dv work graph summary` renders a human-readable table by default.
- [x] `dv work graph summary --format json` returns the same summary facts as
      structured JSON.
- [x] `dv work graph export --format json` returns the whole projected graph with
      schema version, summary metadata, nodes, edges, and diagnostics.
- [x] `dv work graph export --format dot` produces valid directed graph syntax
      suitable for Graphviz rendering.
- [x] Diagnostics remain separate from canonical nodes and edges in export JSON.
- [x] Existing `nodes`, `edges`, and `inspect` commands still work.
- [x] The command surface is read-only and does not create claims, locks,
