import {
  selectReadyTasks,
  type SelectReadyTasksOptions,
} from "../task/ready.js";

/** Stable capability name for publisher-owned Work selection. */
export const PUBLISHED_WORK_SELECTION_CAPABILITY =
  "publisher-work-selection/v1" as const;
...
export interface PublishedWorkSelectionOptions extends Pick<
  SelectReadyTasksOptions,
  "rootDir" | "backlogDir"
> {
...
export function formatPublishedWorkSelectionCommand(options: {
  ...
  return (
    `dv work select ${quoteCommandToken(options.workItemId)} --request ${quoteCommandToken(options.request)}` +
    (options.backlogDir === undefined
      ? ""
      : ` --backlog-dir ${quoteCommandToken(options.backlogDir)}`) +
    (options.json ? " --json" : "")
  );
}
...
function isArtifact(
  ...
  return (
    ...
    (typeof value.requestedWorkItemId === "string" ||
      value.requestedWorkItemId === null) &&
...
export async function selectPublishedWork(
  ...
  if (
    ...
    return response(
      capability,
      { kind: "not-selected", code: "UNSUPPORTED_CAPABILITY" },
      artifact(command, null, null),
    );
  }
  if (!isRequest(request)) {
    return response(
      capability,
      { kind: "not-selected", code: "INVALID_REQUEST" },
      artifact(command, null, null),
    );
  }
  try {
    if (options.authorize && !(await options.authorize(request))) {
      return response(
        capability,
        { kind: "not-selected", code: "NOT_AUTHORIZED" },
        artifact(command, request.request.workItemId, null),
      );
    }
    const ready = await selectReadyTasks({
    ...
    return response(
      capability,
      { kind: "not-selected", code: "NOT_FOUND" },
      evidence,
    );
  } catch {
    return response(
      capability,
      { kind: "not-selected", code: "PUBLISHER_UNAVAILABLE" },
      artifact(command, request.request.workItemId, null),
    );
  }
}
...
export function decodePublishedWorkSelectionResponse(
  ...
  if (
    ...
    (PUBLISHED_WORK_NON_SELECTION_CODES as readonly string[]).includes(
      value.outcome.code,
    )
  ) {
    return {
