...
const request = {
  ...
  request: {
    workItemId: "wi-001",
    invocationContext: { invocation: "fixture" },
  },
};
...
describe("publisher work selection contract", () => {
  ...
  it("retains capability and opaque evidence for a decoded non-selection", async () => {
    const artifact = {
      invokedCommand: "dv work select wi-001 --request - --json" as const,
      sourceResult: Buffer.from(
        JSON.stringify({ sourceSchemaVersion: "task-ready/v1" }),
      ).toString("base64"),
  ...
  it("returns authorization and publisher availability outcomes from publisher-owned options", async () => {
    ...
    ).resolves.toMatchObject({ outcome: { code: "NOT_AUTHORIZED" } });
    await expect(
      selectPublishedWork(request, {
    ...
    await expect(selectPublishedWork(request)).resolves.toMatchObject({
      outcome: { code: "PUBLISHER_UNAVAILABLE" },
    });
  });
  ...
  it.each([
    ...
    [
      "AMBIGUOUS",
      request,
      { ...ready, candidates: [ready.candidates[0]!, ready.candidates[0]!] },
    ],
    [
      "INVALID_REQUEST",
      { capability: PUBLISHED_WORK_SELECTION_CAPABILITY, request: {} },
      undefined,
    ],
    [
      "UNSUPPORTED_CAPABILITY",
      {
        capability: "publisher-work-selection/v2",
        request: { workItemId: "wi-001", invocationContext: {} },
      },
      undefined,
    ],
  ] as const)(
    "publishes %s from publisher-owned selection",
    async (code, fixtureRequest, fixtureReady) => {
      if (fixtureReady)
        vi.mocked(selectReadyTasks).mockResolvedValue(fixtureReady);
      await expect(selectPublishedWork(fixtureRequest)).resolves.toMatchObject({
        capability:
          code === "UNSUPPORTED_CAPABILITY"
            ? fixtureRequest.capability
            : PUBLISHED_WORK_SELECTION_CAPABILITY,
        outcome: { kind: "not-selected", code },
        decisionArtifact: {
          invokedCommand:
            code === "INVALID_REQUEST"
              ? "dv work select unknown --request - --json"
              : "dv work select wi-001 --request - --json",
        },
      });
  ...
  it("retains the exact file or stdin transport invocation as evidence", async () => {
    ...
    await expect(
      selectPublishedWork(request, {
        invokedCommand: "dv work select wi-001 --request request.json --json",
      }),
    ).resolves.toMatchObject({
      decisionArtifact: {
        invokedCommand: "dv work select wi-001 --request request.json --json",
  ...
  it("requires canonical base64-encoded JSON source evidence in the in-process decoder", () => {
    ...
    expect(() =>
      decodePublishedWorkSelectionResponse(request, {
        ...response,
    ...
    expect(() =>
      decodePublishedWorkSelectionResponse(request, {
        ...
        decisionArtifact: {
          ...artifact,
          sourceResult: Buffer.from("not json", "utf8").toString("base64"),
        },
      }),
    ).toThrow();
  });
  ...
  it("uses canonical shell-safe command evidence for whitespace paths", () => {
    ...
    expect(invokedCommand).toBe(
      "dv work select wi-001 --request 'fixtures/request with spaces.json' --backlog-dir 'custom backlog/it'\"'\"'s-ready' --json",
    );
    const artifact = {
      invokedCommand,
      sourceResult: Buffer.from("{}", "utf8").toString("base64"),
    ...
    expect(
      decodePublishedWorkSelectionResponse(request, response),
    ...
    expect(portable.decode(request, response)).toMatchObject({
      kind: "selected",
    });
  });
  ...
  it("accepts official file transport artifacts and rejects unrelated commands", () => {
    ...
    expect(() =>
      decodePublishedWorkSelectionResponse(request, {
        ...
        decisionArtifact: {
          ...artifact,
          invokedCommand: "dv work ready --json",
        },
      }),
    ).toThrow();
  });
    ...
    expect(
      portable.decode(request, {
        ...
        decisionArtifact: {
          ...artifact,
          invokedCommand:
            "dv work select wi-001 --request fixtures/request.json",
        },
      }),
    ).toMatchObject({ kind: "selected" });
    expect(
      portable.decode(request, {
        ...
        decisionArtifact: {
          ...artifact,
          invokedCommand:
            "dv work select wi-001 --request fixtures/request.json --backlog-dir custom-backlog --json",
        },
      }),
    ).toMatchObject({ kind: "selected" });
    expect(() =>
  ...
  it("binds request identity and invocation context consistently in both decoders", () => {
    ...
    for (const decode of [
      ...
      expect(() =>
        decode(request, {
          ...response,
          decisionArtifact: { ...artifact, requestedWorkItemId: null },
        }),
      ).toThrow();
      expect(() =>
        decode(request, {
          ...response,
          decisionArtifact: { ...artifact, requestedWorkItemId: "wi-other" },
        }),
      ).toThrow();
    }
  });
  ...
  it.each([
    ...
    "fails closed for malformed, unsupported, mismatched, or artifact-free responses",
    (response) => {
      expect(() =>
        decodePublishedWorkSelectionResponse(request, response),
      ).toThrow();
